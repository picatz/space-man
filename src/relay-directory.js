/* ============================================================================
   SPACE MAN — RELAY DIRECTORY (Run Together, Addendum E).
   Parses the OFFICIAL relay-directory JSON format NATIVELY (Regions{} → Nodes[]:
   RegionID, RegionCode, RegionName [city label], Latitude, Longitude,
   Nodes[].HostName [, DERPPort]). No bespoke schema, no transformation — every
   ...
   sm2.* namespace and is WRITTEN ONLY when networking is active (load({active:
   true})). Hostnames and endpoint URLs are DATA (honest, devtools-visible),
   never branding.

   REFRESH PROCEDURE (dev-time, Docker only — no host runtimes):
     docker run --rm alpine:3.20 sh -c \
       'apk add -q curl && curl -s <relay directory endpoint>' > derpmap-raw.json
     diff <(old) <(new)          # review the delta
     # paste the new JSON VERBATIM between BAKED-BEGIN / BAKED-END (no reformat,
     # no minify, no key changes — it is a literal copy), then commit with the
     # diff in the message body. Do not hand-edit the blob.
   ========================================================================== */
(function (root) {
  'use strict';
...
`;
  ...
  function parse(obj) {
    ...
    var src = obj && obj.Regions, regions = [];
    if (src) for (var id in src) {
      if (!Object.prototype.hasOwnProperty.call(src, id)) continue;
      var r = src[id];
      if (!r || typeof r.RegionCode !== 'string') continue;
      var hosts = [];
      if (Array.isArray(r.Nodes)) for (var i = 0; i < r.Nodes.length; i++) {
        var n = r.Nodes[i];
        if (n && typeof n.HostName === 'string') {
          var port = (typeof n.DERPPort === 'number' && n.DERPPort >= 1 && n.DERPPort <= 65535 && n.DERPPort !== 443)
            ? ':' + Math.floor(n.DERPPort) : '';
          var h = n.HostName + port;
          if (HOST_RE.test(h)) hosts.push(h);            // malformed host = dropped, never a URL ingredient
        }
      }
    ...
    return { v: 1, src: 'official', regions: regions };
  }

  // SERVER LIST mode: plain hostnames → one synthetic region ("usr").
  function fromServerList(hosts, label) {
    var arr = Array.isArray(hosts) ? hosts : String(hosts || '').split(/[\s,]+/);
    var clean = [];
    for (var i = 0; i < arr.length && clean.length < 4; i++) {
  ...
  // Re-validate a map that crossed a storage boundary. localStorage is writable
  // by ANY same-origin script (github.io project pages share the user origin) —
  // a poisoned cache must not steer relays for 24h. Regions/hosts that fail the
  // shape or host rule are dropped; an empty result falls through to baked.
  function sanitizeMap(m) {
    if (!m || !Array.isArray(m.regions)) return null;
    var regions = [];
    for (var i = 0; i < m.regions.length && regions.length < 40; i++) {
      var r = m.regions[i];
      if (!r || typeof r.code !== 'string' || !Array.isArray(r.hosts)) continue;
      var hosts = [];
      for (var j = 0; j < r.hosts.length && hosts.length < 4; j++) {
        if (typeof r.hosts[j] === 'string' && HOST_RE.test(r.hosts[j])) hosts.push(r.hosts[j]);
      }
      if (!hosts.length) continue;
      regions.push({
    ...
    return regions.length ? { v: 1, src: String(m.src || 'cache').slice(0, 16), regions: regions } : null;
  }

  // Cache: read is free (no side effects); write ONLY when networking is active.
  function cacheRead() {
    try {
      var raw = root.localStorage && root.localStorage.getItem(CACHE_KEY);
      if (!raw) return null;
      var o = JSON.parse(raw);
      if (!o || !o.map || (Date.now() - o.t) > CACHE_TTL) return null;
      return sanitizeMap(o.map);                    // storage is same-origin-writable: never trust shape or hosts
    } catch (e) { return null; }
  }
  function cacheWrite(map, active) {
    if (!active) return;                          // never persist unless a room is live
    try { if (root.localStorage) root.localStorage.setItem(CACHE_KEY, JSON.stringify({ t: Date.now(), map: map })); } catch (e) {}
  }

  // Fallback ladder. mode: 'default' | 'custom' | 'list'. Only load() ever
  ...
  async function load(opts) {
    ...
    if (url && typeof fetch === 'function' && o.fetch !== false) {                 // 1) live
      try {
        ...
        var t = setTimeout(function () { ctl.abort(); }, o.timeout || 6000);
        var res = await fetch(url, { cache: 'no-store', signal: ctl.signal });
        clearTimeout(t);
        if (res && res.ok) {
          var map = parse(await res.json());
          if (map.regions.length) { cacheWrite(map, active); return { map: map, source: 'live' }; }
        }
      } catch (e) { /* fall to cache */ }
    }
    var cached = cacheRead();                                                       // 2) cache
    if (cached && cached.regions && cached.regions.length) return { map: cached, source: 'cache' };
    return { map: baked(), source: 'baked' };                                       // 3) baked verbatim
  }

  root.SpaceManRelayDir = {
    DEFAULT_ENDPOINT: DEFAULT_ENDPOINT, CACHE_KEY: CACHE_KEY, CACHE_TTL: CACHE_TTL,
    parse: parse, fromServerList: fromServerList, baked: baked, load: load,
    cacheRead: cacheRead, cacheWrite: cacheWrite, sanitizeMap: sanitizeMap, BAKED_JSON: BAKED_JSON,
  };
